...
class CellArchitect:
    ...
    def virtual_step(self, train_X, train_y, xi, w_optim):
        ...
        if self.cell_type == "normal":
            gradients = torch.autograd.grad(loss, self.net.normal_weights())
            ...
            with torch.no_grad():
                # dict key is not the value, but the pointer. So original network
                # weight have to be iterated also.
                for w, vw, g in zip(self.net.normal_weights(),
                                    self.v_net.normal_weights(),
                                    gradients):
                    m = w_optim.state[w].get(
                        "momentum_buffer", 0.0) * self.w_momentum
                    vw.copy_(w - xi * (m + g + self.w_weight_decay * w))
                ...
                # synchronize alphas
                for a, va in zip(self.net.normal_alphas(),
                                 self.v_net.normal_alphas()):
                    va.copy_(a)
        else:
            gradients = torch.autograd.grad(loss, self.net.reduce_weights())
            ...
            with torch.no_grad():
                # dict key is not the value, but the pointer. So original network
                # weight have to be iterated also.
                for w, vw, g in zip(self.net.reduce_weights(),
                                    self.v_net.reduce_weights(),
                                    gradients):
                    m = w_optim.state[w].get(
                        "momentum_buffer", 0.0) * self.w_momentum
                    vw.copy_(w - xi * (m + g + self.w_weight_decay * w))
                ...
                # synchronize alphas
                for a, va in zip(self.net.reduce_alphas(),
                                 self.v_net.reduce_alphas()):
                    va.copy_(a)
    ...
    def backward(self, train_X, train_y, val_X, val_y, xi, w_optim):
        ...
        if self.cell_type == "normal":
            # compute gradient
            v_alphas = tuple(self.v_net.normal_alphas())  # reduce_alphas
            v_weights = tuple(self.v_net.normal_weights())  # reduce_weights
            ...
            if self.use_first_order_darts:
                with torch.no_grad():
                    for alpha, da in zip(self.net.normal_alphas(), dalpha):
                        alpha.grad = da
                ...
                # update final gradient = dalpha - xi*hessian
                with torch.no_grad():
                    for alpha, da, h in zip(self.net.normal_alphas(), dalpha, hessian):
                        alpha.grad = da - xi * h
        else:
            # compute gradient
            v_alphas = tuple(self.v_net.reduce_alphas())  # reduce_alphas
            v_weights = tuple(self.v_net.reduce_weights())  # reduce_weights
            ...
            if self.use_first_order_darts:
                with torch.no_grad():
                    for alpha, da in zip(self.net.reduce_alphas(), dalpha):
                        alpha.grad = da
                ...
                # update final gradient = dalpha - xi*hessian
                with torch.no_grad():
                    for alpha, da, h in zip(self.net.reduce_alphas(), dalpha, hessian):
                        alpha.grad = da - xi * h
    ...
    def compute_hessian(self, dw, train_X, train_y):
        ...
        eps = 0.01 / ||dw||
        """
        raise NotImplementedError("Second degree not implemented for NAS env")
