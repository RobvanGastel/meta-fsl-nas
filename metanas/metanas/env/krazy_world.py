...
class TileTypes:
    def __init__(self, rng):
        ...
        self.ice = 8
        self.energy = 9
        self.all_tt = [self.hole, self.normal, self.goal, self.agent,
                       self.transporter, self.door,
                       self.key, self.death, self.ice, self.energy]
        self.initial_colors = [
            Color.black, Color.white, Color.gold, Color.blue,
            Color.orange, Color.silver, Color.magenta, Color.red,
...
class GameGrid:
    ...
    def get_one_non_agent_square(self):
        g = self.task_rng.randint(0, self.grid_squares_per_row - 1, (2,))
        if g[0] != self.agent.agent_position[0] \
                or g[1] != self.agent.agent_position[1]:
            return g
