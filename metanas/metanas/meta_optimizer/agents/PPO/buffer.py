...
class RolloutBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95,
        ...
        self.obs = torch.zeros(
            (self.n_workers, self.worker_steps) + obs_dim.shape)
    ...
    def prepare_batch_dict(self):
        """
            ...
            Due to using a recurrent policy, the data is split into episodes or
        ...
        for key, value in samples.items():
            ...
            # length. Therfore we can train batches of sequences in parallel
        ...
        # long as the longest episode.
        # Hence, this information has to be stored for the mini batch
                # generation.
    ...
    def recurrent_mini_batch_generator(self):
        """A recurrent generator that returns a dictionary providing training
                data arranged in mini batches. This generator shuffles the
                                data by sequences.
        ...
        for i in range(remainder):
            # Add the remainder if the sequence count and the number of mini
            # batches do not share a common divider
            num_sequences_per_batch[i] += 1
        # Prepare indices, but only shuffle the sequence indices and not the
            # entire batch.
        indices = torch.arange(0,
                               num_sequences * self.true_sequence_length).reshape(
            num_sequences, self.true_sequence_length)
        sequence_indices = torch.randperm(num_sequences)
        # At this point it is assumed that all of the available training data
        # (values, observations, actions, ...) is padded.
        ...
        for n_sequences in num_sequences_per_batch:
            ...
            for key, value in self.samples_flat.items():
                if key != "hxs" and key != "cxs":
                    ...
                    # Collect only the recurrent cell states that are at the
